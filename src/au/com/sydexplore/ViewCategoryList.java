	...
	protected void onCreate(Bundle savedInstanceState){
		...
		try {
			...
			for (int i = 0; i < n; ++i) {
				...
				// Try to get the distance from point A to point B
				if (hasLocation){
		...
		if(ViewCategory.attractionsArray.size()>0){
			// Sort the attractions by distance, then by name 
			if (hasLocation==true){
		...
		Thread t = new Thread() {
			...
			public void run() {
				...
				try {
					...
					String line = null;
					// Construct JSON string from response
